...
def gencode(length=10, chars=(string.uppercase+string.digits)):
    return ''.join([choice(chars) for i in range(length)])

def secret_key(n=50):
    return gencode(n, 'abcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*(-_=+)')

def random_token(n=12):
    return gencode(n, (string.letters+string.digits))

passgen = random_token
